...
prop_roundtrip_hPutGetBufSome bs (Small c) =
  BS.length bs >= fromIntegral c ==>
    ioProperty $
      withSystemTempDirectory "prop_roundtrip_hPutGetBufSome" $ \dirPath -> do
        ...
        FS.withFile hfs (FS.mkFsPath ["temp"]) (FS.WriteMode FS.MustBeNew) $ \h -> do
          ...
          let cmpTest =
                counterexample "(prefix of) input and output bytestring do not match" $
...
prop_roundtrip_hPutGetBufSomeAt bs (Small c) off =
  BS.length bs >= fromIntegral c ==>
    ioProperty $
      withSystemTempDirectory "prop_roundtrip_hPutGetBufSomeAt" $ \dirPath -> do
        let hfs = IO.ioHasFS (FS.MountPoint dirPath)
...
prop_roundtrip_hPutGetBufExactly bs (Small c) =
  BS.length bs >= fromIntegral c ==>
    ioProperty $
      withSystemTempDirectory "prop_roundtrip_hPutGetBufExactly" $ \dirPath -> do
        let hfs = IO.ioHasFS (FS.MountPoint dirPath)
        ...
        FS.withFile hfs (FS.mkFsPath ["temp"]) (FS.WriteMode FS.MustBeNew) $ \h -> do
          ...
          let cmpTest =
                counterexample "input and output bytestring do not match" $
...
prop_roundtrip_hPutGetBufExactlyAt bs (Small c) off =
  BS.length bs >= fromIntegral c ==>
    ioProperty $
      withSystemTempDirectory "prop_roundtrip_hPutGetBufExactlyAt" $ \dirPath -> do
        let hfs = IO.ioHasFS (FS.MountPoint dirPath)
