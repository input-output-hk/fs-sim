...
ioHasFS mount =
  HasFS
    ...
    , hSeek = \(Handle h fp) mode o ->
        liftIO $
          rethrowFsError fp $
            F.seek h mode o
    , hGetSome = \(Handle h fp) n ->
        liftIO $
          rethrowFsError fp $
            F.read h n
    , hGetSomeAt = \(Handle h fp) n o ->
        liftIO $
          rethrowFsError fp $
            F.pread h n (unAbsOffset o)
    , hTruncate = \(Handle h fp) sz ->
        liftIO $
          rethrowFsError fp $
            F.truncate h sz
    , hGetSize = \(Handle h fp) ->
        liftIO $
          rethrowFsError fp $
            F.getSize h
    ...
    , hGetBufSome = \(Handle h fp) buf bufOff c -> liftIO $
        rethrowFsError fp $
